...
public class ProdutosController {
	...
	public ResponseEntity<ProdutosResponseDTO> put(@RequestBody @Valid ProdutosPutRequestDTO dto) {
		...
		try {
			...
			if (produto.isEmpty()) {
				response.setStatus(HttpStatus.BAD_REQUEST); // HTTP STATUS CODE 400 (BAD REQUEST)
				...
				response.setStatus(HttpStatus.OK); // HTTP STAUS CODE 200 OK
	...
	public ResponseEntity<List<Produto>> getAll() {
		...
		try {
			...
			// verificando se a lista possui produtos
			if (produtos.size() > 0) {
				// HTTP STATUS 200 (OK)
